defmodule FoodOrderProducao.InterfaceAdapters.Controllers.ProductionController do
  ...
  alias FoodOrderProducao.InterfaceAdapters.Gateways.OrderGateway
  alias FoodOrderProducao.InterfaceAdapters.Gateways.ProductGateway
  ...
  def update_production_status(production_params) do
    ...
    with {:ok, production_dto} <- EventProductionDTO.from_map(production_params),
         {:ok, production} <- EventProductionDTO.to_domain(production_dto),
